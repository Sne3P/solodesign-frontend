...
type ProjectContent = {
  title: string;
  description: string[];
  details: { icon: React.ElementType; title: string; value: string }[];
  gallery: { src: string; alt: string }[];
  cta: { title: string; button: string };
};

const getProjectContent = (id: string): ProjectContent => ({
  title: `Projet ${id}`,
  description: [
  ...
  // Si aucune image n'est renvoyée, on met par défaut placeholder.png
...
const ProjectDetail = ({ params }: { params: { id: string } }) => {
  ...
  // Memoize content for performance and scalability
  ...
  // Gestion du fallback d'image pour la galerie et l'image principale
  ...
  return (
    <ParallaxProvider>
      ...
      >
        <LogoTitle />
        <SocialLinks />
        <MenuButton initialMenuOpen={menuOpen} />
        ...
        <main className="container mx-auto px-4 py-24">
          <ParallaxTitle>{projectContent.title}</ParallaxTitle>
          ...
          <AnimatedSection>
            ...
            >
              <Image
                src={getSafeImageSrc(projectContent.gallery[0]?.src)}
                alt="Image du projet"
                width={1200}
                height={600}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mb-24">
            <AnimatedSection>
              ...
              {projectContent.description.map((desc, i) => (
                <motion.p
                  key={i}
            ...
            <AnimatedSection>
              ...
              >
                {projectContent.details.map((item, index) => (
                  <motion.div
                    key={index}
          ...
          <AnimatedSection>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
              {projectContent.gallery.length === 0 ? (
                ...
                projectContent.gallery.map((img) => (
                  <motion.div
                    key={img.src}
                    className="relative overflow-hidden rounded-lg shadow-xl cursor-pointer"
                    whileHover={{ scale: 1.05, rotate: 1 }}
                    transition={{ type: "spring", stiffness: 300, damping: 10 }}
                    onClick={() => setSelectedImage(getSafeImageSrc(img.src))}
                  >
                    <Image
          ...
          <AnimatedSection>
            <div className="text-center mt-24">
              ...
              >
                <span>{projectContent.cta.button}</span>
                <motion.div
        ...
        <AnimatePresence>
          {selectedImage && (
            ...
            >
              ...
              >
                <Image
                  src={getSafeImageSrc(selectedImage || undefined)}
                  alt="Image agrandie"
                  width={800}
                  height={600}
